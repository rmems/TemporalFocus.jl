...
distractor) has the largest spike count but poor temporal alignment.
"""
...
function _summary_header(io)
    ...
    println(io)
    println(io, "Generated by `experiments/jitter_test.jl`. Measured result numbers below come from ",
        "`metrics.csv`; setup values, sweep grids, and seeds are recorded in `config.toml`.")
    println(io)
...
function _has_steep_reordering(rows, tau, window)
    baseline = only(r for r in rows
        if r.kernel == "continuous" && r.tau == tau && r.window == window && r.jitter == 0.0f0)
    paired = Dict((r.jitter, r.seed) => r for r in rows
        if r.kernel == "continuous" && r.tau == tau && r.window == window && r.jitter > 0.0f0)
    for i in 1:(length(JITTERS) - 1)
        transitions = count(SEEDS) do seed
            # The zero-jitter output is deterministic and stored once, so it
            # is the valid shared baseline for every seed's first transition.
            before = i == 1 ? baseline : paired[(JITTERS[i], seed)]

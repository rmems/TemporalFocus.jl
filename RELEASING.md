...
2. Update **every** place that embeds the package UUID, at least:
   ...
   - the committed root `Manifest.toml` — after changing `Project.toml`, run
     `julia --project=. -e 'using Pkg; Pkg.resolve()'` (not `Pkg.instantiate()`,
     which reuses the existing lockfile) and commit the updated
     `[[deps.TemporalFocus]]` / `project_hash`; otherwise instantiate/CI fail
